...
def generate_machine_data(session, start_date, end_date, mode='overwrite'):
    sensor_values_df = (
        session.table('ML_DB.DEMO_DATA.MACHINE_SENSORS')
            .filter(F.col('SENSOR_TIMESTAMP').between(start_date, end_date))
    )
    
    sensor_values_df.write.save_as_table(table_name='ML_DB.PUBLIC.MACHINE_SENSORS', mode=mode)
    
    targets_df = (
        session.table('ML_DB.DEMO_DATA.MACHINE_FAILURES')
            .filter(F.col('DATE').between(start_date, end_date))
    )
    
    targets_df.write.save_as_table(table_name='ML_DB.PUBLIC.MACHINE_FAILURES', mode=mode)


def setup(session):
    # Remove existing data
    st.write('Removing existing data ...')
    session.sql("CREATE OR REPLACE SCHEMA DEMO_DATA;").collect()
    session.sql("CREATE OR REPLACE SCHEMA ML_FEATURE_STORE;").collect()
    session.sql("CREATE OR REPLACE SCHEMA ML_MODEL_REGISTRY;").collect()
    ...
    # Create initial data
    generate_machine_data(session, start_date='2025-01-01', end_date='2025-04-01')
    st.write('Demo Setop done!')